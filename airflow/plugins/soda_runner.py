...
def run_soda_duckdb(check_file: str, data_source: str = "duckdb"):
    ...
    con.execute(
        "CREATE VIEW pull_requests AS SELECT * FROM read_parquet('s3://lakehouse/raw/github_data/pull_requests/*.parquet');"
    )
    # For issues, handle the case where it might be empty
    try:
        con.execute(
            "CREATE VIEW issues AS SELECT * FROM read_parquet('s3://lakehouse/raw/github_data/issues/*.parquet');"
        )
    except Exception:
